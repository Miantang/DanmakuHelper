	...
	public class CubicBezier {
		...
		public static function toQuadratics(c:CubicBezier):Array {
			...
			for(var n:int = 0; n <= 3; n++)
				quads.push(new QuadraticBezier(controls[n], anchors[n], anchors[n+1]));
			return quads;
		}
		...
		private static function limitPrecision(n:Number):Number{
			return NumberUtils.limitPrecision(n, scriptManager.limit);
		}
		...
		private static function getBaseLines(c:CubicBezier):Array {
			...
			return [ new Line(c.p1, c.c1), new Line(c.c1, c.c2), new Line(c.c2, c.p2) ];//P0- - -P3
		}
		...
		private static function getSubLines(c:CubicBezier, ratio:Number):Array {
			var subLines:Array = [];
			var baseLines:Array = getBaseLines(c);
			subLines[0] = new Line(Line.getPoint(baseLines[0], ratio), Line.getPoint(baseLines[1], ratio));//p01 - p12
			subLines[1] = new Line(Line.getPoint(baseLines[1], ratio), Line.getPoint(baseLines[2], ratio));//p12 - p23
			subLines[2] = new Line(Line.getPoint(subLines[0], ratio), Line.getPoint(subLines[1], ratio));//Pa - pb
